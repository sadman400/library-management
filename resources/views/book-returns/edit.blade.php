...
@section('content')
    ...
    <div class="card">
        <div class="card-body">
            <form action="{{ route('book-returns.update', $return->borrow_detail_id) }}" method="POST">
                @csrf
                @method('PUT')
                <div class="row">
                    <div class="col-md-6 mb-3">
                        ...
                        <select class="form-select @error('book_id') is-invalid @enderror" id="book_id" name="book_id" required>
                            ...
                            @foreach($books as $book)
                                <option value="{{ $book->book_id }}" {{ old('book_id', $return->book_id) == $book->book_id ? 'selected' : '' }}>
                    ...
                    <div class="col-md-6 mb-3">
                        ...
                        <select class="form-select @error('borrow_id') is-invalid @enderror" id="borrow_id" name="borrow_id" required>
                            ...
                            @foreach($issuances as $issuance)
                                <option value="{{ $issuance->borrow_id }}" {{ old('borrow_id', $return->borrow_id) == $issuance->borrow_id ? 'selected' : '' }}>
                    ...
                    <div class="col-md-6 mb-3">
                        ...
                        <select class="form-select @error('borrow_status') is-invalid @enderror" id="borrow_status" name="borrow_status" required>
                            <option value="Returned" {{ old('borrow_status', $return->borrow_status) == 'Returned' ? 'selected' : '' }}>Returned</option>
                            <option value="Damaged" {{ old('borrow_status', $return->borrow_status) == 'Damaged' ? 'selected' : '' }}>Damaged</option>
                            <option value="Lost" {{ old('borrow_status', $return->borrow_status) == 'Lost' ? 'selected' : '' }}>Lost</option>
                        </select>
                        @error('borrow_status')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    ...
                    <div class="col-md-6 mb-3">
                        ...
                        <input type="date" class="form-control @error('date_return') is-invalid @enderror" id="date_return" name="date_return" value="{{ old('date_return', $return->date_return ? date('Y-m-d', strtotime($return->date_return)) : '') }}" required>
                        @error('date_return')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
